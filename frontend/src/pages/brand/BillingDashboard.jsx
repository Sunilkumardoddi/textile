...
import {
    CreditCard, ArrowLeft, Calendar, TrendingUp, Zap, Crown, CheckCircle,
    AlertTriangle, ArrowUpRight, Download, RefreshCw, Shield, Package,
    Building2, ChevronRight, Percent
} from 'lucide-react';
...
const PLAN_META = {
    brand_starter:      { name: 'Starter',      color: 'blue',    icon: Zap,        posLimit: 10,  suppLimit: 5  },
    brand_professional: { name: 'Professional', color: 'emerald', icon: TrendingUp, posLimit: 50,  suppLimit: 25 },
    brand_enterprise:   { name: 'Enterprise',   color: 'purple',  icon: Crown,      posLimit: null, suppLimit: null },
};
...
const MOCK_USAGE = {
    pos_used: 7,
    suppliers_connected: 4,
    docs_uploaded: 23,
    audits_requested: 2,
};

const MOCK_INVOICES = [
    { id: 'INV-2027-004', date: '2027-04-01', amount: 149, status: 'paid',   description: 'Professional Plan — April 2027' },
    { id: 'INV-2027-003', date: '2027-03-01', amount: 149, status: 'paid',   description: 'Professional Plan — March 2027' },
    { id: 'INV-2027-002', date: '2027-02-01', amount: 149, status: 'paid',   description: 'Professional Plan — February 2027' },
    { id: 'INV-2027-001', date: '2027-01-01', amount: 149, status: 'paid',   description: 'Professional Plan — January 2027' },
];
...
export default function BillingDashboard() {
    ...
    useEffect(() => {
        try {
            ...
            if (stored && stored.role === 'brand') {
                setSub(stored);
            } else {
                // Default demo subscription
                setSub({
                    planId: 'brand_professional',
    ...
    const meta = PLAN_META[sub.planId] ?? PLAN_META.brand_professional;
    const c = COLOR_MAP[meta.color];
    const PlanIcon = meta.icon;
    const isCommission = sub.cycle === 'commission';
    ...
    const handleUpgrade = () => {
        navigate('/subscribe', { state: { role: 'brand', isNew: false } });
    };
        ...
        <div className="min-h-screen bg-slate-900 p-6">
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                >
                    ...
                    {sub.planId === 'brand_enterprise' ? 'Manage Plan' : 'Upgrade Plan'}
                </Button>
            </div>
            ...
            <div className={`rounded-2xl border p-6 mb-6 ${c.bg} ${c.border}`}>
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {sub.status === 'active' && sub.planId !== 'brand_enterprise' && (
                ...
                {[
                    {
                        ...
                        value: `${MOCK_USAGE.pos_used}${meta.posLimit ? ` / ${meta.posLimit}` : ''}`,
                        icon: Package,
                        color: 'blue',
                        progress: meta.posLimit ? (MOCK_USAGE.pos_used / meta.posLimit) * 100 : null,
                    },
                    {
                        label: 'Suppliers Connected',
                        value: `${MOCK_USAGE.suppliers_connected}${meta.suppLimit ? ` / ${meta.suppLimit}` : ''}`,
                        icon: Building2,
                        color: 'emerald',
                        progress: meta.suppLimit ? (MOCK_USAGE.suppliers_connected / meta.suppLimit) * 100 : null,
                    },
                    {
                    ...
                    },
                    {
                        ...
                        value: MOCK_USAGE.audits_requested,
                        icon: CheckCircle,
                        color: 'amber',
                        progress: null,
                    },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <Card className="bg-slate-800 border-slate-700">
                    ...
                    <CardContent className="space-y-3">
                        {[
                            { label: 'Billing cycle', value: isCommission ? 'Commission (per transaction)' : sub.cycle === 'yearly' ? 'Annual' : 'Monthly' },
                            { label: 'Current plan', value: `${meta.name} — ${sub.price}` },
                            ...(nextDate && !isCommission ? [{ label: 'Next billing date', value: nextDate }] : []),
                            ...(isCommission ? [{ label: 'Commission rate', value: sub.price }] : []),
                ...
                <Card className="bg-slate-800 border-slate-700">
                    ...
                    <CardContent>
                        <div className="space-y-2 mb-4">
                            {(sub.planId === 'brand_professional' || sub.planId === 'brand_enterprise'
                                ? ['Full traceability suite', 'AI Style Engine', 'Supply Chain Command Center', 'Audit & compliance tools', 'Advanced analytics', 'Priority support']
                                : ['Basic traceability tracking', 'Season management', 'Document uploads', 'Email support']
                            ).map(f => (
                ...
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
                    <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 max-w-sm w-full">
                        ...
                        <p className="text-slate-400 text-sm mb-6">
                            Your access will continue until the end of the current billing period. After that, your account will be downgraded.
                        </p>
